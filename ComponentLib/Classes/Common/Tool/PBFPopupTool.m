...
@interface PBFPopupTool () <UIGestureRecognizerDelegate>
...
@property (nonatomic, assign) BOOL isTapMaskNotDismiss;//点击Mask是否 不消失popView

@end

@implementation PBFPopupTool
...
+(instancetype)popupControllerWithLayoutType:(PopupLayoutType)layoutType
    ...
    PBFPopupTool *popupController = [[PBFPopupTool alloc] init];
...
- (void) keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        CGFloat marge = (kScreenHeight - _popupView.height)/2;
        CGFloat textMaxY = kScreenHeight/2 + _popupView.height/2;
        CGFloat keybordY = kScreenHeight - kbHeight;
        if (keybordY < textMaxY) {
            CGPoint center = _popupView.center;
            center.y = center.y - (kbHeight - marge)/2;
            _popupView.center = center;
        }
